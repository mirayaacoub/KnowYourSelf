...
export function BookPage() {
  ...
  if (isLoggedIn) {
    useEffect(() => {
      ...
      getTherapistById(Number(therapistId)).then(therapist => {
        // Fetch therapist schedule
        console.log(therapist.therapist_id);
        return getScheduleByTherapist(therapist.therapist_id);
      })
        .then(response => {
          if (isMounted && Array.isArray(response.schedule)) { // Check if schedule is an array
            console.log(response.schedule)
            setScheduleData(response.schedule); // set schedule data with array of schedules
            const availableSchedules = response.schedule.filter((s: Schedule) => s.isBooked === 0);
          ...
          getPatient(userObj.user_id)
            .then(patient => {
              ...
              bookSchedule(scheduleId, patient.patient_id)
                ...
                .catch(error => {
                  // Handle error
                  setDialogMessage('There was an error while booking.\nPlease try again');
                  setShowDialog(true);
                  // alert('Booking error')
  ...
  const maxDate = availableDates.length > 0 ? availableDates[availableDates.length - 1] : new Date(new Date().getFullYear() + 1, 11, 31); // Set maximum date to the newest available date, or one year from now if there are no available dates
        ...
        <div className="w-1/2 h-1/2 flex items-center justify-center">
          <div className="p-8">
            <DateTimePicker
              ...
              disabledDates={availableDates.filter(date => date.getTime() !== minDate.getTime() && date.getTime() !== maxDate.getTime()).map(date => date.toISOString().split('T')[0])} // Pass array of disabled dates (dates other than minDate and maxDate)
            />
            ...
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4" onClick={bookAppointment}>
              Book
            </button>
          </div>

        </div>
        <div className="w-1/2 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url("/booking-image2.jpg")` }}></div>

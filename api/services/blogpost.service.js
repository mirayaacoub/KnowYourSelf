...
const createBlogPost = async (data) => {
    ...
    try {
        ...
        if (res.status === 200) {
            ...
            const blogpost = await BlogPost.create({ therapist_id: therapist.therapist_id, blog_title, content });
...
const getBlogPostsByTherapist = async (data) => {
    ...
    try {
        ...
        if (res.status === 200) {
            // If therapist is found, extract therapist data
            const therapist = res.therapist;
            const blogpost = await BlogPost.findAll({ where: { therapist_id: therapist.therapist_id } });
